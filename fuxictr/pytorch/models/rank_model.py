...
class BaseModel(nn.Module):
    ...
    # def check_nae_parameters(self, lr):
    #     rest_params = []
    #     nae_paras = []
    #     flag = False
    #     for k, v in self.named_parameters():
    #         if 'gamma' not in k or 'nae' not in k:
    ...
    #             {'params': nae_paras, 'lr': lr, 'betas': (0.9, 0.99)}
    #         ]
    #     else:
    #         parameters = [
    #             {'params': rest_params, 'lr': lr},
    ...
    # def compile(self, optimizer, loss, lr):
    #     self.optimizer = get_optimizer(optimizer, self.check_nae_parameters(lr), lr)
    #     self.loss_fn = get_loss(loss)
    ...
    def init_analyzer(self, feature_map, data_format, train_data, valid_data, test_data):
        ...
        # self.analyzer_root_path = os.path.join('/zhdd/home/jcwang/FuxiCTR/embedding', 'raw_data', self.dataset_id, self.model_id)
        # self.analyzer_root_path = os.path.join('/zhdd/home/jcwang/FuxiCTR/test_embedding', 'raw_data', self.dataset_id, self.model_id)
        self.analyzer_root_path = os.path.join('/zhdd/home/jcwang/FuxiCTR/test_embedding', 'percentage_data', self.dataset_id, self.model_id)
        print('Model ID', self.model_id)
        # self.analyzer_root_path = os.path.join(self.model_dir, 'analysis')
    ...
    def analyze_step(self, batch_data, training=False):
        if not training:
            ...
            # print('aaaa', self.feature_embedding_grad.shape)
            # 检测回传给feature embedding的grad
            ...
            # print(self.feature_embedding_grad.grad)
            self.grad_emb_list.append(self.feature_embedding_grad.grad.detach().cpu())
            
            # self.grad_emb_list.append(self.trans_embedding_grad.grad.detach().cpu())




            # self.grad_emb_list.append(self.feature_embedding_grad.grad)
            # self.grad_list.append(('feature_embedding_grad', self.feature_embedding_grad.grad.detach().cpu()))
            # print(self.grad_list)
            # print(self.feature_embedding_grad.shape)
    ...
    def analyze_epoch_end(self, prefix=''):
        ...
        def get_recorded_emb():
            ...
            # print(log_attributes_values.keys())
            # log_attributes_values['grad'] = self.grad_list[0]['feature_embedding_grad']
            
            log_attributes_values = {k: torch.cat(v) for k, v in log_attributes_values.items() if len(v) > 0}
            log_attributes_values['feat_emb_grad'] = self.grad_emb_list[0]
            # print(self.grad_emb_list)
            if len(self.grad_emb_list) > 1:
                log_attributes_values['trans_emb_grad'] = self.grad_emb_list[1]
            # print(log_attributes_values.keys())
            print('Feature_embedding Shape:', log_attributes_values['feature_emb'].shape)
            # print('Final_output Shape:', log_attributes_values['final_out'].shape)
            # 打印grad
            # print('Embedding Grad Shape:', log_attributes_values['emb_grad'][0].shape)
    ...
    def train_epoch(self, data_generator):
        self._batch_index = 0
        # 定义需要存档的百分比节点
        save_checkpoints = [0, 20, 40, 60, 80]
        # 计算对应的 batch 索引
        save_steps = [int(len(data_generator) * (p / 100.0)) for p in save_checkpoints]
        print(save_steps)
        train_loss = 0
        self.train()
        if self._verbose == 0:
        ...
        for batch_index, batch_data in enumerate(batch_iterator):
            ...
            self._total_steps += 1
            if self._epoch_index == 1:
                if batch_index in save_steps:
                    percent = save_checkpoints[save_steps.index(batch_index)]
                    self.save_at_percent(percent)
    ...
    def save_at_percent(self, percent):
        save_path = os.path.abspath(os.path.join(self.model_dir, self.model_id + f'_percent_{percent}' + ".model"))
        print(f"Saving checkpoint at {percent}% of Epoch 1 to {save_path}")
        torch.save(self.state_dict(), save_path)
    
    def load_weights(self, checkpoint):
        self.to(self.device)
        state_dict = torch.load(checkpoint, map_location="cpu", weights_only=True)
        miss_para = self.load_state_dict(state_dict, strict=False)
        try:
            self.nae.feature_gating.weight.data.copy_(state_dict['feature_gating.0.weight'])
            self.nae.feature_gating.bias.data.copy_(state_dict['feature_gating.0.bias'])
        except:
            logging.info("Load NAE weights failed: {}. NAE will not work!".format(miss_para))
